...
class Kernel extends ConsoleKernel
{
    ...
    protected function schedule(Schedule $schedule)
    {
        if (config('app.env') == 'local') {
            ...
            $schedule->command('rocket:take-photo --vehicle-plate=VCK-542')->cron('*/5 * * * *')->between('05:00', '22:00');
//            $schedule->command('rocket:release-busy --vehicle-plate=VCK-542')->hourly()->between('06:00', '18:00');

            $schedule->command('rocket:take-photo --vehicle-plate=VQA-312')->cron('*/2 * * * *')->between('05:00', '22:00');
