...
function getLabelsView(){
    $("label").each(function(){ labelsForValidation[ $(this).attr('for')+'Label' ] = $(this).text(); });
}

//Función para determinar si un elemento es NO NULO
function is_not_null(element){
    if( element == 'null' || element == null || element == undefined || element == '' || element.toString() == '0' ){return false;}
    return true;
}
...
function input_validate(element_id,text_validate){
    ...
    $('body').on('change','#'+element_id+', input[name="'+element_id+'"]', function() {
        if( is_not_null( $(this).val() ) ){is_valid(this);}
    });
    // Validación y alerta:
    var elementValue = $("#"+element_id).val();
    if( elementValue == undefined )elementValue=$('input[name="'+element_id+'"]').val();
    if( is_not_null( elementValue ) ){  $("label[for='"+element_id+"']").css('color',''); return true; }
    else{ $("label[for='"+element_id+"']").css('color','rgba(200, 20, 10, 1)').text('').fadeOut().html('<b>'+text_validate+'</b>').fadeIn(); return false;
    }
}
...
function input_validate_parent(parent,element_id,text_validate){
    ...
    // Evento para cuando el input cambie
    $('body').on('change','#'+element_id+', input[name="'+element_id+'"]', function() {
        if( is_not_null( $(this).val() ) ){is_valid_parent(parent,this);}
    });
