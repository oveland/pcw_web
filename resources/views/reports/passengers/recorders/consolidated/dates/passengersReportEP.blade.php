            ...
            <div id="all-report-tab" class="table-responsive-xs table-report tab-pane active fade in">
                ...
                <table class="table table-bordered table-striped table-hover table-valign-middle">
                    ...
                    <tr class="inverse">
                        ...
                        <th class="text-center">
                            <i class="fa fa-bus" aria-hidden="true"></i><br> @lang('PRUEBA VEHICULO EP')
                        </th>
                        <th class="text-center">
                       ...
                       {{-- @if($company->hasRecorderCounter())
                            ...
                            <th class="p-3 {{ $passengerReport->canLiquidate ? '' : 'hide' }}">
                                <i class="fa fa-file-pdf-o"
                                   aria-hidden="true"></i><br> @lang('Settlement daily receipt')
                            </th>
                        @endif--}}
                    </tr>
                    </thead>
                    ...
                    @php
                        $totalSensor = collect([]);
                        $sumtotalByVIsual = collect([]);
                        $totalAllSensor = collect([]);
                        $totalRecorder = collect([]);
                        $totalPassengerVisual = collect([]);
                    @endphp
                    @foreach($reports as $date => $report)
                        @php
                                $sumtotalByVIsual = 0;
                                ...
                                $sumtotalByVIsual = $report->totalByVIsual;
                                ...
                                $sumtotalByVIsual > 0 ? $totalPassengerVisual->push($sumtotalByVIsual) : null;
                        ...
                        <tr class="text-center click" data-toggle="collapse" data-target="#collapse-{{ $date }}">
                           ...
                           {{-- <td class="recorder text-center">
                            ...
                            @endif
                            <td class="sensor">{{ $report->totalByVIsual }}</td>
                            <td class="sensor">{{ number_format($report->totalByVIsual * $tariff, 0, ',', '.') }} </td>
                           {{-- @if($company->hasSensorCounter())
                    ...
                    <tr class="inverse bg-inverse text-white">
                        <td colspan="4" class="text-right text-uppercase">@lang('Totals')</td>
...
{{--                        <td class="text-center recorder">{{ number_format($passengerReport->totalRecorder, 0) }}</td>--}}
                        ...
                        @endif
                        <td class="text-center sensor">{{ $totalPassengerVisual->sum() }}</td>
                        <td class="text-center sensor">{{ number_format($totalPassengerVisual->sum() * $tariff, 0, ',', '.') }} </td>
                       {{-- @if($company->hasSensorCounter())
