                        ...
                        @foreach($historySeats as $historySeat)
                            <div class="col-md-12 p-0">
                                ...
                                <div class="bg-white p-0" style="height: auto;padding: 5px;">
                                    ...
                                    @endif
                                    <div class="progress progress-striped p-0 m-0 no-rounded-corner progress-{{$loop->first?'lg':'sm'}}" style="opacity: {{ $loop->first ? 1 : 0.8 }};height: {{ $loop->first ? 40 : 8 }}px !important;">
                                        @php($parentLoop = $loop)
                                        @php($width = 0)
                                        @foreach($controlPoints as $controlPoint)
                                            @php($width = ($controlPoint->distance_next_point*100/$routeDistance) - 1.3 )
                                            @php($trajectory = $controlPoint->name . ' ➤ '.($loop->index+1<count($controlPoints)?$controlPoints[$loop->index + 1]->name : '') )
                                            <div class="progress-bar bg-inverse-{{ $loop->index%2 == 0 ? 'dark' : 'light' }} p-t-5" style="width:{{ number_format(( $width ),'1','.','') }}%; font-size: 120%"
